...
def find_file_by_name(base_path, filename):
    """
    Search for a file in the directory and its subdirectories that matches the given filename.
...
def extract_expiring_employees():
    ...
    # Find specific files based on the filenames
    demographics_file = find_file_by_name(base_path, demographics_filename)
    audit_file = find_file_by_name(base_path, audit_filename)
    ...
    print(f"Found Absolute Employee Demographics.xlsm at: {demographics_file}")
    ...
    try:
        ...
        print(f"Failed to open Absolute Employee Demographics.xlsm: {e}")
